...
# Everything below writes pixel bytes to stdout; redirect once instead of
# wrapping the whole render loop in a `{ ... } > "$OUT"` block, which would
# add a whole extra indent level on top of the loop/if/case nesting below.
exec > "$OUT"
...
for ((py = HEIGHT - 1; py >= 0; py--)); do  # BMP stores rows bottom-up
		...
		for ((px = 0; px < WIDTH; px++)); do
				...
				row_iter_sum=$((row_iter_sum + iter))
				...
				# Per-pixel terminal spam — written to stderr so it never lands
				# in the BMP (stdout is redirected to $OUT above).
				printf 'px(%4d,%4d) iter=%3d/%-3d %-7s rgb=(%3d,%3d,%3d)\n' \
						"$px" "$py" "$iter" "$MAX_ITER" "$status" "$r" "$g" "$b" >&2
				bmp::rgb "$r" "$g" "$b"
		done
		bmp::pad "$padding"
		((in_set_total += row_in_set))
		((iter_sum_total += row_iter_sum))
done

total_pixels=$((WIDTH * HEIGHT))
in_set_pct=$((in_set_total * 100 / total_pixels))
avg_iter=$((iter_sum_total / total_pixels))
echo "Done in $((SECONDS - _start))s — ${total_pixels} pixels, ${in_set_pct}% in set, avg iterations ${avg_iter}/${MAX_ITER}" >&2
